...
export const LoginForm: React.FC = () => {
  ...
  const { login, loading, error, user, clearError, initializeFromStorage } = useAuthStore()
  
  useEffect(() => {
    const initialize = async () => {
      try {
        const hasStoredAuth = await initializeFromStorage()
        if (hasStoredAuth) {
          navigate('/dashboard')
        }
      } catch (err) {
    ...
    initialize()
  }, [initializeFromStorage, navigate])
